...
<head>
    <meta charset="UTF-8">
    <title>Electronics - Cyrokart</title>
...
.sidebar {
 ...
 .sidebar button {
    ...
    .dark-mode .wishlist-highlighted {
        background-color: #9C006F !important;
    }
    ...
    <div class="products">
        <%
            try {
                ...
                // Get wishlist items
                PreparedStatement wishStmt = conn.prepareStatement("SELECT product_id FROM wishlist WHERE customer_id = ?");
                wishStmt.setInt(1, customerId);
                ResultSet wishRs = wishStmt.executeQuery();
                java.util.HashSet<Integer> wishlistIds = new java.util.HashSet<>();
                while (wishRs.next()) {
                    wishlistIds.add(wishRs.getInt("product_id"));
                }
...
<div class="product-card" style="<%= inWishlist ? "background-color:#ffe6e6;" : "" %>">
    ...
    <button 
        ...
        <%
                ...
                out.println("Error loading electronics products: " + e.getMessage());
            }
        %>
    </div>
</div>
...
<!-- Toast -->
<div class="toast" id="toast">Added to your Wishlist ❤️</div>
...
function showToast(message) {
    ...
    if (existing) existing.remove(); // Remove any old toast
