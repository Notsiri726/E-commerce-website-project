...
<head>
    <meta charset="UTF-8">
    <title>General Products - Cyrokart</title>
    ...
    <div class="products">
        <%
            try {
                ...
                PreparedStatement wishlistStmt = conn.prepareStatement("SELECT product_id FROM wishlist WHERE customer_id = ?");
                wishlistStmt.setInt(1, customerId);
                ResultSet wishlistRs = wishlistStmt.executeQuery();
                while (wishlistRs.next()) {
                    wishlistIds.add(wishlistRs.getInt("product_id"));
                }
                wishlistRs.close();
                wishlistStmt.close();

                // Get general products that are not hot deals
                ...
                while (rs.next()) {
                    ...
                    boolean inWishlist = wishlistIds.contains(productId);
        %>
        <div class="product-card <%= inWishlist ? "wishlist-highlighted" : "" %>">
            ...
            <button 
                class="wishlist-btn" 
                onclick="addToWishlist(<%= productId %>, this)" 
        ...
        <%
                ...
                out.println("Error loading general products: " + e.getMessage());
            }
        %>
    </div>
</div>
